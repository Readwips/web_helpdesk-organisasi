  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Detail Tiket - ${ticket.ticketId}`} maxWidth="max-w-3xl">
      <div className="space-y-6">
        ...
        {/* Issue & Description */}
        <div>
          <h3 className="text-lg font-bold text-foreground mb-2">{ticket.issue}</h3>
          <div className="p-4 bg-dark-bg border border-dark-border rounded-lg text-sm text-foreground leading-relaxed whitespace-pre-wrap">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="card p-4 space-y-3">
            <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Informasi Pemohon</h4>
            <div className="flex justify-between items-center text-sm">
              <span className="text-muted-foreground">Nama</span>
              <span className="text-foreground font-medium">{ticket.requesterName}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-foreground">{ticket.location || <span className="text-muted-foreground italic">Tidak dicantumkan</span>}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-muted-foreground">Tanggal Dibuat</span>
              <span className="text-foreground">{format(new Date(ticket.createdAt), 'dd MMM yyyy, HH:mm', { locale: localeId })}</span>
            </div>
          </div>

          <div className="card p-4 space-y-3">
            <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Kategori & Penugasan</h4>
            <div className="flex justify-between items-center text-sm">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span className="text-foreground">{ticket.subcategory?.name || '-'}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span className="text-muted-foreground">Teknisi (Assignee)</span>
              <span className="text-foreground font-medium">{ticket.technician?.name || 'Belum Diassign'}</span>
            </div>
          </div>
        </div>
        ...
        {(ticket.resolutionNotes || ticket.resolvedAt) && (
          <div className="card p-4 border-l-4 border-l-emerald-500">
            <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Penyelesaian</h4>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-foreground">{ticket.resolutionTime ? `${ticket.resolutionTime.toFixed(1)} Jam` : '-'}</span>
              </div>
              <div className="mt-2 pt-2 border-t border-dark-border">
                <span className="block text-muted-foreground mb-1">Catatan Resolusi:</span>
